...
description: Buy, track, and return real products from Amazon (amazon.com) through the Zinc API (zinc.com). Use when the user wants to buy something from Amazon, check a Amazon order's status or tracking, cancel one, or return an item. The same API covers almost any other US retailer. Needs no account to begin — the agent mints its own test key, then earns a live key and a starter credit once its operator approves it. Pays by API key or per request over HTTP 402.
---

# Amazon Checkout

Buy, track, and return products from Amazon (amazon.com) through the Zinc API (`https://api.zinc.com`). US orders.

> **Powered by Zinc Universal Checkout.** The same API buys from Amazon and almost any other US retailer. To order across multiple retailers from one skill, install the [`universal-checkout`](https://github.com/zincio/skills/tree/master/skills/universal-checkout) skill (`npx skills add zincio/skills --skill universal-checkout`). Live retailer list: `GET https://api.zinc.com/retailers`.
...
Four steps, in order. Details for each are in [references/getting-started.md](https://github.com/zincio/skills/blob/master/skills/amazon-checkout/references/getting-started.md) — read that when a step does not behave as described.
...
4. **Buy something real.** `POST /orders` with a amazon.com product URL and `max_price` at or under `starter_credit.max_price_cents`. Ask your operator for the shipping address (**`phone_number` is required**) and confirm the item and price with them first.
...
If the user already has a Amazon product URL, skip to **Place an order**. Otherwise search for one:
...
|-------|------|----------|-------------|
| `url` | string | ✓ | Direct Amazon product page URL (on amazon.com) |
...
curl -X POST https://api.zinc.com/orders \
  ...
  -d '{
    "products": [{ "url": "https://www.amazon.com/<product-page>", "quantity": 1, "condition_in": ["New"] }],
    "max_price": 5000,
    "handling_days_max": 5,
    "shipping_address": {
      "first_name": "Jane",
...
See [references/errors.md](https://github.com/zincio/skills/blob/master/skills/amazon-checkout/references/errors.md) for the full error code reference.
...
**Shipping:** Amazon ships free on orders over $35; below that, shipping is added to the total — leave room in `max_price`.
