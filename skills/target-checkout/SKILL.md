...
5. **Buy something real from Target.** Ask your operator for a shipping address — **`phone_number` is required** — and never reuse the sandbox address. Then `POST /orders` with a target.com product URL and `max_price` at or under `starter_credit.max_price_cents`. Confirm the item and the price with your operator before placing a live order.
