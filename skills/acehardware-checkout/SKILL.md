...
description: Buy, track, and return real products from Ace Hardware (acehardware.com) through the Zinc API (zinc.com). Use when the user wants to buy something from Ace Hardware, check a Ace Hardware order's status or tracking, cancel one, or return an item. The same API covers almost any other US retailer. Needs no account to begin — the agent mints its own test key, then earns a live key and a starter credit once its operator approves it. Pays by API key or per request over HTTP 402.
---

# Ace Hardware Checkout

Buy, track, and return products from Ace Hardware (acehardware.com) through the Zinc API (`https://api.zinc.com`). US orders.

> **Powered by Zinc Universal Checkout.** The same API buys from Ace Hardware and almost any other US retailer. To order across multiple retailers from one skill, install the [`universal-checkout`](https://github.com/zincio/skills/tree/master/skills/universal-checkout) skill (`npx skills add zincio/skills --skill universal-checkout`). Live retailer list: `GET https://api.zinc.com/retailers`.
...
Four steps, in order. Details for each are in [references/getting-started.md](https://github.com/zincio/skills/blob/master/skills/acehardware-checkout/references/getting-started.md) — read that when a step does not behave as described.
...
4. **Buy something real.** `POST /orders` with a acehardware.com product URL and `max_price` at or under `starter_credit.max_price_cents`. Ask your operator for the shipping address (**`phone_number` is required**) and confirm the item and price with them first.
...
If the user already has a Ace Hardware product URL, skip to **Place an order**. Otherwise search for one:
...
`GET /search` returns `{ status, query, results: [...] }` across retailers; each result has a directly **orderable `url`** plus `retailer`, `title`, `price` (cents), `stars`. Filter results to `retailer == "acehardware"` for Ace Hardware-only, then pass the `url` into an order.
...
|-------|------|----------|-------------|
| `url` | string | ✓ | Direct Ace Hardware product page URL (on acehardware.com) |
...
curl -X POST https://api.zinc.com/orders \
  ...
  -d '{
    "products": [{ "url": "https://www.acehardware.com/<product-page>", "quantity": 1, "condition_in": ["New"] }],
    "max_price": 5000,
    "handling_days_max": 5,
    "shipping_address": {
      "first_name": "Jane",
...
async with Client(methods=[method]) as client:
    response = await client.post(
        ...
        json={
            "products": [{"url": "https://www.acehardware.com/<product-page>", "quantity": 1}],
            "max_price": 5000,
            "shipping_address": {
...
See [references/errors.md](https://github.com/zincio/skills/blob/master/skills/acehardware-checkout/references/errors.md) for the full error code reference.
