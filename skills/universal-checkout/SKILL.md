...
4. **Buy something real.** Walmart is the best-covered retailer, so search there first: `GET /search?q=<item> walmart`. **`max_price` is a ceiling, not a price** — you are charged the retailer's actual total, so set it a little above the listed price to leave room for tax and shipping. Ask your operator for the shipping address (**`phone_number` is required**) and confirm the item and price with them first.
