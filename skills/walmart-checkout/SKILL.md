...
description: Buy products from Walmart (walmart.com) and manage those orders via the Zinc API (zinc.com). Use when the user wants to purchase, order, or check out an item from Walmart, check Walmart order status or tracking, cancel a Walmart order, or return a Walmart item. One API also covers Amazon, Walmart, Target, Best Buy and 50+ other US retailers. No account needed to start — an agent can mint its own sandbox key, then collect its own live key and a starter credit after one browser approval from its operator. Supports API key auth (ZINC_API_KEY) or Machine Payments Protocol (MPP) for per-request payments via a Stripe card, Tempo stablecoins, or x402 (USDC on Base).
---
...
5. **Buy something real from Walmart.** Ask your operator for a shipping address — **`phone_number` is required** — and never reuse the sandbox address. Then `POST /orders` with a walmart.com product URL and `max_price` at or under `starter_credit.max_price_cents`. Confirm the item and the price with your operator before placing a live order.
