...
description: Buy, track, and return real products from Walmart (walmart.com) through the Zinc API (zinc.com). Use when the user wants to buy something from Walmart, check a Walmart order's status or tracking, cancel one, or return an item. The same API covers almost any other US retailer. Needs no account to begin — the agent mints its own test key, then earns a live key and a starter credit once its operator approves it. Pays by API key or per request over HTTP 402.
---

# Walmart Checkout

Buy, track, and return products from Walmart (walmart.com) through the Zinc API (`https://api.zinc.com`). US orders.

> **Powered by Zinc Universal Checkout.** The same API buys from Walmart and almost any other US retailer. To order across multiple retailers from one skill, install the [`universal-checkout`](https://github.com/zincio/skills/tree/master/skills/universal-checkout) skill (`npx skills add zincio/skills --skill universal-checkout`). Live retailer list: `GET https://api.zinc.com/retailers`.
...
Four steps, in order. Details for each are in [references/getting-started.md](https://github.com/zincio/skills/blob/master/skills/walmart-checkout/references/getting-started.md) — read that when a step does not behave as described.
...
4. **Buy something real.** `POST /orders` with a walmart.com product URL and `max_price` at or under `starter_credit.max_price_cents`. Ask your operator for the shipping address (**`phone_number` is required**) and confirm the item and price with them first.
...
If the user already has a Walmart product URL, skip to **Place an order**. Otherwise search for one:
...
`GET /search` returns `{ status, query, results: [...] }` across retailers; each result has a directly **orderable `url`** plus `retailer`, `title`, `price` (cents), `stars`. Filter results to `retailer == "walmart"` for Walmart-only, then pass the `url` into an order.
...
Walmart is one of the few retailers with richer product data (currently Amazon & Walmart only). For best-price comparison, use `GET /products/search?query=<term>&retailer=walmart` (returns `product_id`, `price`, `ship_price`, `stars`, …) and `GET /products/{product_id}/offers?retailer=walmart` to compare offers by **price and condition** before ordering. On the MPP rail these are `POST /agent/products/search`, `POST /agent/products/offers`, and `POST /agent/products/details` (query param `product_id=…&retailer=walmart`), $0.01 per call.
...
|-------|------|----------|-------------|
| `url` | string | ✓ | Direct Walmart product page URL (on walmart.com) |
...
See [references/errors.md](https://github.com/zincio/skills/blob/master/skills/walmart-checkout/references/errors.md) for the full error code reference.
...
**Shipping:** Walmart ships free on orders over $35; below that, shipping is added to the total — leave room in `max_price`.
