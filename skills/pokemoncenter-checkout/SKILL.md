...
Four steps, in order. Details for each are in [references/getting-started.md](https://github.com/zincio/skills/blob/master/skills/pokemoncenter-checkout/references/getting-started.md) — read that when a step does not behave as described.
...
4. **Buy something real.** `POST /orders` with a pokemoncenter.com product URL and `max_price` at or under `starter_credit.max_price_cents`. Ask your operator for the shipping address (**`phone_number` is required**) and confirm the item and price with them first.
...
curl -X POST https://api.zinc.com/orders \
  ...
  -d '{
    "products": [{ "url": "https://www.pokemoncenter.com/<product-page>", "quantity": 1, "condition_in": ["New"] }],
    "max_price": 5000,
    "handling_days_max": 5,
    "shipping_address": {
      "first_name": "Jane",
...
See [references/errors.md](https://github.com/zincio/skills/blob/master/skills/pokemoncenter-checkout/references/errors.md) for the full error code reference.
